...
func test_register_miss_if_extra_hit_within_buffer():
	...
	asserts.is_false(bp.hit(9.5), "Miss should be registered if extra hit done on within the same buffer")
...
func test_beat_on_given_pattern():
	...
	var tests := {
		...
		[1, 2, 1]: [
			...
			{
				"desc": "Advanced 2: Miss should be registered on second pattern sol in first cycle",
				"time": 0.5,
				"want": false,
			},
		],
		[1, 2, 1, 2, 1]: [
			{
				"desc": "Moving offbeat: Miss should be registered on second pattern sol in first cycle",
				"time": 0.5,
				"want": false,
			},
			{
				"desc": "Moving offbeat: Miss should be registered on fourth pattern sol in first cycle",
				"time": 1.25,
				"want": false,
			},
			{
